...
## [1.6.0] - 2026-03-29 - Новые MCP инструменты и исправления

### Добавлено
- **4 новых MCP инструмента**:
  - `get_examples` - Примеры использования кода для элемента
  - `get_methods` - Список методов объекта
  - `get_properties` - Список свойств объекта
  - `get_events` - Список событий объекта

### Исправлено
- **LogContext**: Переход на `contextvars` для thread-safety
- **Cache**: Исправлены стратегии LRU/LFU
- **Config**: Исправлены типы, добавлена CORS-конфигурация
- **ES client**: Добавлены публичные свойства `client` и `config`
- **hbk_parser**: Улучшен asyncio в cleanup

### Изменено
- Улучшена обработка UTF-8 в ответах
- Добавлена Gzip компрессия ответов
